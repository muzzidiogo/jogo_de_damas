...
bool Engine::conta_jogadas(Tabuleiro_t _tabuleiro, Peca_t _peca) {
    ...
    if (linha - 2 >= 0 && coluna - 2 >= 0 &&  //se a jogada puder ser feita
      !_tabuleiro.verificar_posicao({linha - 2, coluna - 2}) && //e houver espaço para a captura 
    ...
    if (linha + 2 < 8 && coluna + 2 < 8 && //0 a 7, 8 pecas. 8 é o max + 1  
      !_tabuleiro.verificar_posicao({linha + 2, coluna + 2}) &&
...
void Engine::roda_engine(Tabuleiro_t _tabuleiro, char _cor) {
  for (unsigned int i = 0; i < _tabuleiro.get_tabuleiro().size(); i++) {
    if (_tabuleiro.get_tabuleiro()[i].get_cor() == _cor) {
      ...
      this->conta_jogadas(_tabuleiro, _tabuleiro.get_tabuleiro()[i]);
